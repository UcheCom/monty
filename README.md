...
* This project is a custom ByteCode interpreter, that's coded in C. It reads from the file "uche_file.m" that contains one instruction per line. Then the right function is called to modify the stack going by the instruction. Custom error messages are printed if the instruction is wrong.
